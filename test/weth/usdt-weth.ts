...
describe("USDT -- WETH", function () {
    ...
    mocha.step("покупка на 100 usdt weth в разных обмениках", async function() {
        ...
        const sushiBal = await WETH.balanceOf(wbnbRecipient.address);
        ...
        const pancakeBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(sushiBal);
        ...
        const biBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal);
        ...
        const mdexBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal);
        ...
        const apeBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal);
        ...
        const babyBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal);
        ...
        const nomiBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal).sub(babyBal);
        ...
        const knightBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal).sub(babyBal).sub(nomiBal);
        ...
        const bakeryBal = (await WETH.balanceOf(wbnbRecipient.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal).sub(babyBal).sub(nomiBal).sub(knightBal);
