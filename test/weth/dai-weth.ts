...
describe("DAI -- WETH", function () {
    ...
    mocha.step("Buying BUSD and USDT", async function() {
        ...
        const daiBal = await DAI.balanceOf(usdSource.address);
        console.log('daiBal', daiBal.div(divider));
    });

    mocha.step("Checking dai amount for each DEX", async function() {
        const amountIn = ethers.utils.parseUnits('100', 18);
        const path = pathes.daiToWeth;
