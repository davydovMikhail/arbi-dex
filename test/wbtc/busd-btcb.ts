...
describe("BUSD -- BTCB", function () {
    ...
    mocha.step("Buying BUSD", async function() {
    ...
    mocha.step("Checking btcb amount for each DEX by busd", async function() {
    ...
    mocha.step("покупка на 100 busd weth в разных обмениках", async function() {
        ...
        const biBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal);
        ...
        const mdexBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal);
        ...
        const apeBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal);
        ...
        //     timestamp + 90
        // );
        // const babyBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal);
        ...
        const nomiBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal);
        ...
        const knightBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal).sub(nomiBal);
        ...
        //     timestamp + 90
        // );
        // const bakeryBal = (await BTCB.balanceOf(wbnbRecipient2.address)).sub(pancakeBal).sub(sushiBal).sub(biBal).sub(mdexBal).sub(apeBal).sub(nomiBal).sub(knightBal);
